...
const tbl = prisma.tbl_verification_status_master;


const getAllVerificationStatus = async (req, res) => {
  try {
    const allRecords = await tbl.findMany({
      select: { id: true, status_name: true, status: true },
      where: { is_deleted: 0 },
      orderBy: { id: "desc" },
    });
...
const addVerificationStatus = async (req, res) => {
  const { status_name } = req.body;

  try {
    const existing = await tbl.findFirst({
      where: { status_name, is_deleted: 0 },
    });

    if (existing) {
      return res.status(200).json({
        success: false,
        message: "VerificationStatus already exists",
        data: {},
      });
    }
 ...
 */
const getAjaxVerificationStatus = async (req, res) => {
  ...
  const columns = ["status_name", "status"];
  ...
  if (searchValue) {
    whereClause = {
      ...whereClause,
      status_name: { contains: searchValue },
    };
  }
...
const uniqueVerificationStatus = async (req, res) => {
  const { status_name, currentId } = req.query;

  try {
  const whereClause = { status_name, is_deleted: 0 };
    if (currentId) {
...
module.exports = {
  ...
  getAjaxVerificationStatus,
  uniqueVerificationStatus,
};
