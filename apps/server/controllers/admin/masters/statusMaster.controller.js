...
const tbl = prisma.tbl_status_master;
...
const addStatus = async (req, res) => {
  const { service_name, status_name } = req.body;
...
/**
 * AJAX listing (pagination, search, sorting)
 */
const getAjaxStatus = async (req, res) => {
  ...
  const columns = ["service_name", "status_name", "status"];
  ...
  if (searchValue) {
    whereClause = {
      ...whereClause,
      OR: [
        { service_name: { contains: searchValue } },
 ...
 * Unique status check
 */
const uniqueStatus = async (req, res) => {
  const { service_name, status_name, currentId } = req.query;
  ...
  try {
    ...
    const existing = await tbl.findFirst({ where: whereClause });
