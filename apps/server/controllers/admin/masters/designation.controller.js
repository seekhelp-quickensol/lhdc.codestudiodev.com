...
const tbl = prisma.tbl_designation_master;
...
const getAjaxDesignation = async (req, res) => {
  ...
  if (searchValue) {
    whereClause = {
      ...whereClause,
      designation_name: { contains: searchValue },
    };
  }
...
const uniqueDesignation = async (req, res) => {
  const { designation_name, currentId } = req.query;

  try {
  const whereClause = { designation_name, is_deleted: 0 };
    if (currentId) {
