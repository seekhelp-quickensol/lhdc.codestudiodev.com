...
const tbl = prisma.tbl_property_variant_master;


const getAllPropertyVariant = async (req, res) => {
  try {
    const allRecords = await tbl.findMany({
      select: { id: true, property_variant: true, status: true },
      where: { is_deleted: 0 },
      orderBy: { id: "desc" },
    });
...
const addPropertyVariant = async (req, res) => {
  const { property_variant } = req.body;

  try {
    const existing = await tbl.findFirst({
      where: { property_variant, is_deleted: 0 },
    });
...
const getPropertyVariantById = async (req, res) => {
  const { id } = req.params;
...
const updatePropertyVariant = async (req, res) => {
  const { id } = req.params;
  const { property_variant } = req.body;
 ...
 */
const getAjaxPropertyVariant = async (req, res) => {
  ...
  const columns = ["property_variant", "status"];
  ...
  if (searchValue) {
    whereClause = {
      ...whereClause,
      property_variant: { contains: searchValue },
    };
  }
...
const uniquePropertyVariant = async (req, res) => {
  const { property_variant, currentId } = req.query;

  try {
  const whereClause = { property_variant, is_deleted: 0 };
    if (currentId) {
...
module.exports = {
  getAllPropertyVariant,
  addPropertyVariant,
  getPropertyVariantById,
  updatePropertyVariant,
  getAjaxPropertyVariant,
  uniquePropertyVariant,
};
