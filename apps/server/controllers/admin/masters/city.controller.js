...
const tbl = prisma.tbl_city_master;

const getAllCity = async (req, res) => {
  try {
    const allRecords = await tbl.findMany({
      select: { id: true,  state_id:true, city_name: true, status: true , state:true },
      where: { is_deleted: 0 },
      orderBy: { id: "desc" },
    });
...
const addCity = async (req, res) => {
  ...
  try {
    const existing = await tbl.findFirst({
      where: { city_name, state_id: Number(state_id), is_deleted: 0 },
    });
...
const getAjaxCity = async (req, res) => {
  ...
  const columns = [ "state_id",  "city_name", "status"];
  ...
  if (searchValue) {
    whereClause = {
      ...whereClause,
      OR: [
        { city_name: { contains: searchValue } },
...
const uniqueCity = async (req, res) => {
  const { state_id , city_name, currentId } = req.query;

  try {
  const whereClause = {state_id:Number(state_id), city_name, is_deleted: 0 };
    if (currentId) {
