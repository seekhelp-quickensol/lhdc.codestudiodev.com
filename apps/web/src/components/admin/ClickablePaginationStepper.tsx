...
interface Props {
  steps: Step[];
  activeStep: number;
  onStepClick?: (stepId: number) => void; // ✅ NEW
}

export default function ClickablePaginationStepper({
  steps,
  activeStep,
  onStepClick,
}: Props) {
  ...
  return (
    <div className="relative bg-white rounded-xl border shadow-sm p-3">
      {/* ===== STEPPER ===== */}
      <div className="stepper-scroll max-w-7xl  flex gap-4 overflow-x-auto scroll-smooth snap-x snap-mandatory px-2 pb-2">
        {steps.map((step) => {
          ...
          return (
            <div
              ...
              onClick={() => onStepClick?.(step.id)} // ✅ CLICK
              className={`
                snap-center
